...
#                         # Validate weights basic format
#                         valid, reason, norms = validate_weight_list(received_weights)
#                         if not valid:
#                             logger.warning(f"  Update from {key} failed validate_weight_list: {reason}. Storing metadata only.")
#                             client_updates[key] = {'weights': None, 'metadata': metadata, 'validation_reason': reason}
#                         else:
#                             # If initial model exists, ensure shapes match
#                             if initial_weights is not None:
#                                 if not shapes_match(received_weights, initial_weights):
#                                     logger.warning(f"  Update from {key} has shape mismatch vs server model; rejecting update.")
...
#                             else:
#                                 # no initial model to compare shapes to — accept but warn
#                                 client_updates[key] = {'weights': received_weights, 'metadata': metadata, 'norms': norms}
#                                 logger.info(f"  Received weights (layers: {len(received_weights)}) from {key} (no local model to validate shapes)")
...
#                         except Exception as e:
#                             logger.error(f"Failed to calculate divergence: {e}", exc_info=True)
...
#                         except Exception as e:
#                             logger.error(f"Failed to set/save global model weights: {e}", exc_info=True)
#                     else:
...
# fedguard_server.py
"""
FedGuard Secure Server (Fixed - Architecture Compatible).
