...
global_model_insecured= tf.keras.models.load_model(MODEL_PATH)
...
for message in consumer:
    ...
    if len(client_updates) >= NUM_CLIENTS:
        ...
        print("  - RUNNING INSECURE MODE: Accepting all clients...")
        honest_clients_weights = client_weights_list
       ...
       # (Keep all the code before this the same)
        ...
        if not honest_clients_weights:
            ...
            global_model_insecured.set_weights(averaged_weights)
            global_model_insecured.save('global_model_insecured.h5')
